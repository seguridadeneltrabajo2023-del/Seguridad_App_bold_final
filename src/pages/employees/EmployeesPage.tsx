...
export default function EmployeesPage() {
  ...
  // Lógica de búsqueda funcional por cualquier palabra/letra
  ...
  const exportToPdf = () => {
    try {
      ...
      const tableColumn = ["Nombres", "Apellidos", "Tipo ID", "Número ID", "Cargo", "Área", "F. Ingreso", "Estado"];
      ...
      autoTable(doc, {
        ...
        theme: 'grid',
        styles: { fontSize: 8 },
        headStyles: { fillColor: [37, 99, 235], textColor: 255, fontStyle: 'bold' } // Corregido: fillColor en lugar de fillStyle
      });
...
function EmployeeTable({ employees, onAdd, onEdit, onDelete, onChangeStatus, searchTerm, setSearchTerm, onExportExcel, onExportPdf }: any) {
  return (
          ...
          <tbody className="divide-y divide-gray-50">
            {employees.length > 0 ? employees.map((emp: any) => (
              <tr key={emp.id} className="hover:bg-blue-50/30 transition-colors group text-xs">
                <td className="px-6 py-4 font-bold text-gray-800 uppercase">{emp.names}</td>
                <td className="px-6 py-4 font-bold text-gray-800 uppercase">{emp.last_names}</td>
                <td className="px-6 py-4 text-center uppercase">{emp.type_id}</td>
                <td className="px-6 py-4 font-mono">{emp.num_id}</td>
                <td className="px-6 py-4 font-bold text-gray-700">
                  <div className="flex items-center gap-1 text-gray-700 font-bold"><Briefcase size={12} className="text-gray-400" /> {emp.job_title}</div>
                </td>
                <td className="px-6 py-4 text-[10px] uppercase text-gray-500">
                   <div className="flex items-center gap-1 text-gray-500 uppercase"><Building size={10} className="text-gray-400" /> {emp.area || 'General'}</div>
                </td>
                <td className="px-6 py-4 text-xs text-center">
                  <div className="flex items-center justify-center gap-1 text-gray-500"><Calendar size={12} className="text-gray-400" /> {emp.entry_date}</div>
                </td>
...
function EmployeeForm({ onBack, employeeToEdit }: { onBack: () => void, employeeToEdit?: any }) {
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    const dataToSave = { ...formData, children_count: Number(formData.children_count), age, antiquity_months: antiquity };
    try {
  ...
  return (
    <div className="p-4 md:p-8 space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        ...
        <h2 className="text-xl font-black text-gray-800 uppercase tracking-tight flex items-center gap-2"><UserPlus size={20} className="text-blue-600"/> {employeeToEdit ? 'Editar Trabajador' : 'Registro de'} Trabajador</h2>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-blue-600 border-l-4 border-blue-600 pl-2 mb-4"><User size={18} /> <h3 className="font-black text-xs uppercase">Perfil</h3></div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="relative"><label className={labelClass}>Lugar Expedición</label><div className="relative"><FileText className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} /><input type="text" name="expedicion_place" value={formData.expedicion_place} onChange={handleChange} className={`${inputClass} pl-9`} /></div></div>
          </div>
          <div className="grid grid-cols-2 gap-2">
            <div><label className={labelClass}>Nacimiento</label><div className="relative"><Calendar size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/><input type="date" name="birth_date" value={formData.birth_date} onChange={handleChange} className={`${inputClass} pl-9`} /></div></div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-red-600 border-l-4 border-red-600 pl-2 mb-4"><Activity size={18} /> <h3 className="font-black text-xs uppercase tracking-widest">Ubicación y Salud</h3></div>
          <div className="bg-gray-50 p-3 rounded-xl border border-dashed border-gray-200 space-y-3">
             <div className="flex items-center gap-2 text-[9px] font-bold text-gray-400 uppercase"><MapPin size={12}/> Nacimiento</div>
             <div className="grid grid-cols-2 gap-2">
          ...
          <div className="bg-gray-50 p-3 rounded-xl border border-dashed border-gray-200 space-y-3">
             <div className="flex items-center gap-2 text-[9px] font-bold text-gray-400 uppercase"><Home size={12}/> Residencia</div>
             <div className="grid grid-cols-2 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
             <div className="relative"><label className={labelClass}>Dirección</label><div className="relative"><Home className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14}/><input type="text" name="residence_address" value={formData.residence_address} onChange={handleChange} className={`${inputClass} pl-9`} /></div></div>
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2 text-emerald-600 border-l-4 border-emerald-600 pl-2 mb-4"><Briefcase size={18} /> <h3 className="font-black text-xs uppercase tracking-widest">Laboral</h3></div>
          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <input type="text" name="eps" value={formData.eps} onChange={handleChange} placeholder="EPS" className={inputClass} />
            <input type="text" name="afp" value={formData.afp} onChange={handleChange} placeholder="AFP" className={inputClass} />
            <input type="text" name="ccf" value={formData.ccf} onChange={handleChange} placeholder="CCF" className={inputClass} />
          </div>
      ...
      <div className="pt-6 flex justify-end gap-3">
        ...
        <button onClick={handleSave} disabled={loading} className="bg-blue-600 text-white px-10 py-3 rounded-xl font-bold hover:bg-blue-700 flex items-center gap-2 shadow-lg transition-all uppercase text-xs">{loading ? 'Procesando...' : <><Save size={18} /> Guardar Trabajador</>}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-sm rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
        ...
        <p className="text-center text-gray-500 text-sm mb-6">Nuevo estado para <span className="font-bold text-gray-800">{employeeName}</span></p>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm animate-in fade-in">
      <div className="bg-white w-full max-w-md rounded-2xl shadow-2xl p-6 animate-in zoom-in-95">
        ...
        <p className="text-gray-500 text-center text-sm mb-6 font-medium">Vas a borrar permanentemente a <span className="font-bold text-gray-800">{employeeName}</span>.</p>
        <div className="mb-6">
          <label className="text-[10px] font-black text-gray-400 uppercase block text-center mb-2 font-mono text-xs">Escribe "ELIMINAR" para confirmar</label>
